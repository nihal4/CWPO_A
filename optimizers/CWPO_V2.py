...
def CWPO(objf, lb, ub, dim, SearchAgents_no, Max_iter, levy_lambda=1.7):
    ...
    for l in range(Max_iter):
        alpha = 1.0 * (1 - (l / Max_iter))  # Dynamic step size for improved local update
        ...
        for i in range(SearchAgents_no):
            ...
            if best_fitness_in_range < Alpha_score:
                # Improved Local Update Equation
                Positions[i] = Alpha_pos + (alpha * np.random.rand()) * (Alpha_pos - Positions[i])
            else:
