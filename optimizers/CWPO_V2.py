...
def CWPO(objf, lb, ub, dim, SearchAgents_no, Max_iter, initial_lambda=1.5, final_lambda=3.0):
    ...
    for l in range(Max_iter):
        # Dynamically adjust `levy_lambda`
        levy_lambda = initial_lambda + (final_lambda - initial_lambda) * (l / Max_iter)
        ...
        for i in range(SearchAgents_no):
            ...
            # Dynamically adjust the step size using fitness comparison
            if fitness < Alpha_score:
                # Use Equation (1): Move towards the best position with a random influence
                Positions[i] = Alpha_pos + levy_step * (Alpha_pos - Positions[i]) + np.random.rand(dim)
            else:
                # Use Equation (2): Move using Levy flight and random influence
